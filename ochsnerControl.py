...
def get_battery_soc():
    avg_minutes = 5
    influx_db = "solaranzeige"
    influx_query = {'q': "select mean(Bat_Act_SOC) from Batterie where time > '" + str(cre_request_date_minutes(avg_minutes)) + "'"}
    r = requests.get(url=influx_url1 + influx_db, params=influx_query).json()
...
##Check for the last time the temperature was more then 60°
...
def get_wp_state():
    ...
    influx_db = "test1"
    boiler_device = "192.168.1.56"
    influx_query = {'q': "select last(power) from mystrom2 where device='" + boiler_device + "' and time > '" + str(cre_request_date_minutes(avg_minutes)) + "'"}
    r = requests.get(url=influx_url1 + influx_db, params=influx_query).json()
    ...
    avg_minutes = 360
    if wp_state == True:
        influx_query = {'q': "select last(power) from mystrom2 where device='" + boiler_device + "' and power < 100 and time > '" + str(cre_request_date_minutes(avg_minutes)) + "'"}
        r = requests.get(url=influx_url1 + influx_db, params=influx_query).json()
...
def stop_heating():
    ...
    if get_wp_state()[1] == True and get_wp_state()[0] == True:
        print("Turn WP OFF")
        logging.info("Turn WP OFF")
        shelly_boiler.relay(0, turn=False)
    elif get_wp_state()[1] == True and get_wp_state()[0] == False:
...
def decide_what_2_do():
    ...
    if  room_last_peak_value <= 19:
        state_room = 1
    elif room_last_peak_value > 19 and room_last_peak_value < 23:
        state_room = 2
    ...
    if wp_last_temp_value <= 53:
        state_wp = 1
    elif wp_last_temp_value > 53.1 and wp_last_temp_value <= 55.1:
        state_wp = 2
    elif wp_last_temp_value > 55.2 and wp_last_temp_value <= 57.1:
        state_wp = 3
    elif wp_last_temp_value > 57.1 and wp_last_temp_value <= 60.1:
        state_wp = 4
    ...
    if state_wp <= start_condition_wp or days_since_last_legionella.days > 9:
        if state_pv >= start_condition_pv and state_room >= start_condition_room and wp_last_temp_value < desired_water_temp:
            condition="Heating"
